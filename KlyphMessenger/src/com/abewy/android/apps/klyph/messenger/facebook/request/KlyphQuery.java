...
public abstract class KlyphQuery implements RequestQuery
{
	...
	@Override
	public String getQuery(List<GraphObject> previousResults, String id,
			String offset)
	{
		return "";
	}
	...
	@Override
	public List<GraphObject> handleResult(List<GraphObject> previousResults,
			JSONArray result)
	{
		return null;
	}
	...
	protected void assocData(JSONArray initData, JSONArray addData,
			String initKey, String addKey, String initValue, String addValue)
	{
		assocData(initData, addData, initKey, addKey, initValue, addValue,
				null, null);
	}
					...
					if (initObject.optJSONArray(initKey) != null)
					{
						...
						for (int k = 0; k < initArray.length(); k++)
						{
							...
							if (fromKeyValue != null
									&& fromKeyValue.equals(toKeyValue))
							{
								initObject.accumulate(initValue,
										addObject.get(addValue));
	...
	protected void assocData2(JSONArray initData, JSONArray addData,
			String initKey, String addKey, String initValue, String addValue,
				...
				for (int j = 0; j < addData.length(); j++)
				{
					...
					if (initObject.optJSONArray(initKey) != null)
					{
						...
						for (int k = 0; k < initArray.length(); k++)
						{
									...
									&& fromKeyValue.equals(toKeyValue))
							{
								...
								if (initType != null)
									initObject.accumulate(initType,
											addObject.get(addType));
							}
						}
					}
					else
					{
								...
								&& fromKeyValue.equals(toKeyValue))
						{
							...
							if (initType != null)
								initObject
										.put(initType, addObject.get(addType));
						}
					}
				}
			}
	...
	protected void assocStreamToLikedPages(JSONArray streams, JSONArray pages)
	{
		for (int i = 0; i < streams.length(); i++)
		{
			...
			JSONObject objectTags = initObject
					.optJSONObject("description_tags");
	...
	protected void assocStreamToObjectBySubPostId(JSONArray left,
			JSONArray right, String rightId, String leftParam)
	{
		int n = left.length();
		int m = right.length();
		for (int i = 0; i < n; i++)
	...
	protected void assocStreamToObjectById(JSONArray left,
			JSONArray right, String leftId, String rightId, String leftParam)
	{
		int n = left.length();
		int m = right.length();
		for (int i = 0; i < n; i++)
